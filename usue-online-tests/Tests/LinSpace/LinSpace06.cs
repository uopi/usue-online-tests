...
{
    public class LinSpace06 : ITestCreator, ITest, ITestGroup, ITimeLimit
    {
        public int TestID { get; set; }
        public string Name { get; } = "Базис подпространства01.2";
        ...
        public ITest CreateTest(int randomSeed)
        {
            ...
            result.Text =
                ...
                $"\\(\\left\\{{ {letterA}(x) \\mid {Cy}\\cdot {letterA}({Cx}) + " +
                $"{1-Cy} \\cdot {letterA}({Dx}) = 0\\right\\}}\\): \n" +
        ...
        public int CheckAnswer(int randomSeed, Dictionary<string, string> answers)
        {
            ...
            if (answers.TryGetValue("form1", out var va) && int.TryParse(va, out var va1) && va1 == (Cy*Cx+(1-Cy)*Dx)) total++;
            if (answers.TryGetValue("form2", out var va2) && int.TryParse(va2, out var va21) && va21 == (Cy*Cx*Cx+(1-Cy)*Dx*Dx)) total++;
            if (answers.TryGetValue("form3", out var va3) && int.TryParse(va3, out var va31) && va31 == Cy*Cx*Cx*Cx+(1-Cy)*Dx*Dx*Dx) total++;
