...
{
    public class LinSpace08 : ITestCreator, ITest, ITestGroup, ITimeLimit
    {
        public int TestID { get; set; }
        public string Name { get; } = "Базис подпространства01.4";
        ...
        public ITest CreateTest(int randomSeed)
        {
            ...
            result.Text =
                ...
                $"\\(\\left\\{{ {letterA}(x) \\mid {letterA}'({Cx})=0\\right\\}}\\): \n" +
                $"\\(\\left\\{{ 1 - <form1:3>\\cdot x, x^{{2}} - <form2:3>\\cdot x, x^{{3}}-<form3:3>\\cdot x \\right\\}}\\).";
        ...
        public int CheckAnswer(int randomSeed, Dictionary<string, string> answers)
        {
            ...
            if (answers.TryGetValue("form1", out var va) && int.TryParse(va, out var va1) && va1 == 0) total++;
            if (answers.TryGetValue("form2", out var va2) && int.TryParse(va2, out var va21) && va21 == (2*Cx)) total++;
            if (answers.TryGetValue("form3", out var va3) && int.TryParse(va3, out var va31) && va31 == (3*Cx*Cx)) total++;
