...
{
    public class LinSpace07 : ITestCreator, ITest, ITestGroup, ITimeLimit
    {
        public int TestID { get; set; }
        public string Name { get; } = "Базис подпространства01.3";
        ...
        public ITest CreateTest(int randomSeed)
        {
            ...
            int Cx = values[rand.Next(0, values.Count)];
            values.Remove(Cx);
            ...
            ITest result = new LinSpace07(); // Cx Dx Dy
            result.Text =
                ...
                $"\\(\\left\\{{ {letterA}(x) \\mid {letterA}({Cx}) + {Dx}" +
                $"\\cdot {letterA}'({Cx}) + {Dy}\\cdot {letterA}''({Cx}) = 0\\right\\}}\\): \n" +
        ...
        public int CheckAnswer(int randomSeed, Dictionary<string, string> answers)
        {
            ...
            if (answers.TryGetValue("form1", out var va) && int.TryParse(va, out var va1) && va1 == (Dx+Cx)) total++;
            if (answers.TryGetValue("form2", out var va2) && int.TryParse(va2, out var va21) && va21 == (2*Dy+2*Cx*Dx+Cx*Cx)) total++;
            if (answers.TryGetValue("form3", out var va3) && int.TryParse(va3, out var va31) && va31 == (Cx*Cx*Cx+3*Cx*Cx*Dx+6*Cx*Dy)) total++;
