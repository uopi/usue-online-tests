...
{
    public class LinSpace10 : ITestCreator, ITest, ITestGroup, ITimeLimit
    {
        public int TestID { get; set; }
        public string Name { get; } = "Базис подпространства01.5";
        ...
        public ITest CreateTest(int randomSeed)
        {
            ...
            result.Text =
                ...
                $"\\(\\left\\{{ {letterA}(x) \\mid {letterA}({Cx}) + {Dx}\\cdot {letterA}''({Dy})=0\\right\\}}\\): \n" +
        ...
        public int CheckAnswer(int randomSeed, Dictionary<string, string> answers)
        {
            ...
            if (answers.TryGetValue("form1", out var va) && int.TryParse(va, out var va1) && va1 == Cx) total++;
            if (answers.TryGetValue("form2", out var va2) && int.TryParse(va2, out var va21) && va21 == (Cx*Cx-2*Dx)) total++;
            if (answers.TryGetValue("form3", out var va3) && int.TryParse(va3, out var va31) && va31 == (Cx*Cx*Cx-6*Dx*Dy)) total++;
