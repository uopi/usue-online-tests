...
{
    public class BoolFunctA13 : ITestCreator, ITest, ITestGroup
    {
        public int TestID { get; set; }
        public string Name { get; } = "Нормальная дизъюнктивная форма01";
        public string Description { get; } = "Нормальная дизъюнктивная форма";
        ...
        public ITest CreateTest(int randomSeed)
        {
            ...
            ITest result = new BoolFunctA13();
            result.Text =
                "Задайте совершенной нормальной дизъюнктивной формой функцию, заданную таблицей" +
                " истинности (дизъюнкты должны быть в круглых скобках, не переставляйте дизъюнкты местами и не" +
                " переставляйте местами конъюнкты, \\(\\lor\\) вводите как" +
                " строчную букву v, \\(\\land\\) вводите как знак &, пробелов в строке быть не должно):\n " +
        ...
        public int CheckAnswer(int randomSeed, Dictionary<string, string> answers)
        {
            ...
            foreach (var line in table)
            {
                ...
                {
                    answ += "(";
                    answ += a == 1 ? $"{letterA}" : $"-{letterA}";
                    answ += "&";
                    answ += b == 1 ? $"{letterB}" : $"-{letterB}";
                    answ += "&";
                    answ += c == 1 ? $"{letterC}" : $"-{letterC}";
                    answ += ")v";
                }
            }
